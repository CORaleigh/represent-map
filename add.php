...
//exit;
// validate fields
if(empty($title) || empty($type) || empty($address) || empty($uri) || empty($description) || empty($owner_name)) {
  ...
  if($sg_enabled) {
    ...
    $insert = mysql_query("INSERT INTO places (lat, lng, approved, title, type, address, uri, description, owner_name, owner_email, num_employees) VALUES ($lat, $lng, 1, '$title', '$type', '$address', '$uri', '$description', '$owner_name', '$owner_email', '$employeenum')") or die(mysql_error());
    ...
    $insert = mysql_query("INSERT INTO places (lat, lng, approved, title, type, address, uri, description, owner_name, owner_email, num_employees) VALUES ($lat, $lng, null, '$title', '$type', '$address', '$uri', '$description', '$owner_name', '$owner_email', '$employeenum')") or die(mysql_error());
