...
if($task == "doedit") {
  ...
  if(empty($title) || empty($type) || empty($address)) {
  echo "All fields are required - please try again.";
  exit;
  }

  mysql_query("UPDATE places SET title='$title', type='$type', address='$address', uri='$uri', lat='$lat', lng='$lng',
  ...
  //mysql_query("UPDATE places SET title='$title' WHERE id='$place_id' LIMIT 1");
  //geocode
